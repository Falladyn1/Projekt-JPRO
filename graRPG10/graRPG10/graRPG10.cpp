    ...
    for (int i = 0; i < inv->height; i++) {
        for (int j = 0; j < inv->width; j++) {
            if (inv->items[i][j] != NULL &&
                (j == 0 || inv->items[i][j] != inv->items[i][j - 1]) &&
                (i == 0 || inv->items[i][j] != inv->items[i - 1][j])) {
                free(inv->items[i][j]);
            }
        }
    }
    ...
    for (int y = 0; y < inv->height; y++) {
        ...
        for (int x = 0; x < inv->width; x++) {
            if (inv->slots[y][x] == 0) {
                ...
                // Wyświetl symbol tylko w lewym górnym rogu przedmiotu
                if (item != NULL && item->posX == x && item->posY == y) {
                    printf("[%c]", item->symbol);
                }
                else if (inv->slots[y][x] == 1) {
                    printf(" # ");
                }
                else {
                    printf(" . ");
